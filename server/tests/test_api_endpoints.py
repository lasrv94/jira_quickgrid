...
def test_archivy_notes():
    ...
    res = client.get("/api/archivy/notes/CORE-101")
    assert res.status_code == 200
    data = res.json()
    assert "CORE-101" in data["content"]

    # 2. Save custom note
    save_res = client.post("/api/archivy/notes/CORE-101", json={"content": "# Custom RCA\nRoot cause identified."})
    assert save_res.status_code == 200

    # 3. Fetch again and confirm update
    fetch_again = client.get("/api/archivy/notes/CORE-101")
    assert fetch_again.status_code == 200
    assert "Custom RCA" in fetch_again.json()["content"]
